...
class ViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let profile = viewModel.profile(at: indexPath.row)
    ...
    @objc func declineAction(_ sender: UIButton) {
        let profile = viewModel.profile(at: sender.tag)
        viewModel.updateStatus(for: profile.id ?? "", status: "declined")
    }

    @objc func acceptAction(_ sender: UIButton) {
        let profile = viewModel.profile(at: sender.tag)
        viewModel.updateStatus(for: profile.id ?? "", status: "accepted")
    }
